...
def main():
    ...
    for row_index, record in enumerate(records):
        ...
        for method, switches in ARMS.items():
            ...
            save_bundle_metadata(bundle, method_dir / "trajectory.json"); save_bundle_tensors(bundle, method_dir / "trajectory.pt")
            (method_dir / "branch_scores.json").write_text(json.dumps(bundle.branch_records, ensure_ascii=False, indent=2, default=str) + "\n", encoding="utf-8")
            for stage, images in enumerate(bundle.branch_images, 1):
    ...
    print(json.dumps({"output": str(out), "samples": len(records), "rows": len(all_rows)}, indent=2))
